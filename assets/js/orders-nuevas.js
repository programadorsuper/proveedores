(function () {
    ...
    function fetchOrders() {
        ...
        fetch(`${config.endpoints.list}?${params.toString()}`, {
            ...
            .catch((error) => {
                ...
                setTableMessage(error.message || 'No fue posible cargar la informacion.');
            })
            .finally(() => {
                state.loading = false;
                updatePager();
    ...
    function renderTable(rows) {
        ...
        rows.forEach((row) => {
            ...
            tr.innerHTML = `
                <td>
                    <div class="fw-bold text-dark">#${row.FOLIO}</div>
                    <div class="text-muted small">ID ${row.ID_COMPRA}</div>
                </td>
                <td>
                    <div class="fw-semibold text-dark">${escapeHtml(row.RAZON_SOCIAL)}</div>
                    <div class="text-muted small">Proveedor ${escapeHtml(row.NUMERO_PROVEEDOR || '')}</div>
                </td>
                <td>
                    <div class="fw-semibold text-dark">${escapeHtml(row.NOMBRE_CORTO || 'Sin tienda')}</div>
                    <div class="text-muted small">${row.ID_TIENDA_CONSIGNADA ? 'Consignada #' + row.ID_TIENDA_CONSIGNADA : 'Central'}</div>
                </td>
                <td>
                    <div class="fw-semibold">${formatDate(row.FECHA)}</div>
                    <div class="text-muted small">${escapeHtml(row.SERIE || '')}</div>
                </td>
                <td class="text-end fw-bold">${formatMoney(row.TOTAL)}</td>
                <td class="text-center">${row.DIAS_CREDITO || 0} dias</td>
                <td>
                    <div class="text-muted" style="max-width: 260px">${escapeHtml(row.LUGAR_ENTREGA || '')}</div>
                </td>
                <td>${renderSeen(row.seen)}</td>
                <td class="text-end">
                    <div class="btn-group orders-table-actions">
                        <a class="btn btn-sm btn-primary" href="${buildDetailUrl(row.ID_COMPRA)}" target="_blank" rel="noopener" data-action="detail" data-order="${row.ID_COMPRA}" data-provider="${row.ID_PROVEEDOR}">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">
                            <button class="dropdown-item" data-action="download" data-format="pdf" data-order="${row.ID_COMPRA}" data-provider="${row.ID_PROVEEDOR}"><i class="fa-solid fa-file-pdf me-2"></i>Descargar PDF</button>
                            <button class="dropdown-item" data-action="download" data-format="xml" data-order="${row.ID_COMPRA}" data-provider="${row.ID_PROVEEDOR}"><i class="fa-solid fa-code me-2"></i>Descargar XML</button>
                            <button class="dropdown-item" data-action="download" data-format="csv" data-order="${row.ID_COMPRA}" data-provider="${row.ID_PROVEEDOR}"><i class="fa-solid fa-file-csv me-2"></i>Descargar CSV</button>
                            <button class="dropdown-item" data-action="download" data-format="xlsx" data-order="${row.ID_COMPRA}" data-provider="${row.ID_PROVEEDOR}"><i class="fa-solid fa-file-excel me-2"></i>Descargar XLSX</button>
                        </div>
                    </div>
                </td>
            `;
