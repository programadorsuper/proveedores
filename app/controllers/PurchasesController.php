...
class PurchasesController extends ProtectedController
{
    ...
    {
        $this->renderPurchasesPage('Compras (Sell-in)', 'overview');
    }

    public function periods(): void
    {
        $this->renderPurchasesPage('Compras por periodo', 'periods');
    }

    public function sellin(): void
    {
        $this->renderPurchasesPage('Sell-in', 'sellin');
    }

    protected function renderPurchasesPage(string $title, string $page): void
    {
        if (!$this->ensureModule('purchases')) {
            return;
        }
        ...
        $summary = [];
        $trend = ['categories' => [], 'series' => [['name' => 'Sell-in', 'data' => []]]];
        $products = [];
        $stores = [];
        $suggestions = [];
        ...
        if ($providerId > 0) {
            ...
            $trend = $this->analytics->sellinTimeseries($providerId, $filters);
            $products = $this->analytics->sellinProducts($providerId, $filters);
